...
The `resources` directory contains all files that are needed for the workflow, but not specific for an experiment/run. Most of the necessary files are downloaded automatic, including reference genomes, adapter sequences, and blacklists. The directory is structured as follows:
